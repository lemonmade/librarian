...
var extractParams = function (code) {
  var cursor = code.indexOf('(') + 1;
  var length = code.length;
  var lastParamIndex = cursor;
  var depth = 1;
  var params = [];
  var inString = false;
  var inComment = false;
  ...
  while (cursor < length && depth > 0) {
    ...
    if (!(inString || inComment)) {
      if (c === '(') {
        ...
        inList = depth > 1;
        if (depth === 0) {
          params.push(code.substring(lastParamIndex, cursor))
        }
      }
    }

    if (!(inString || inComment || inList)) {
      if (c === ',') {
        params.push(code.substring(lastParamIndex, cursor))
        lastParamIndex = cursor + 1;
      }
    }
  ...
  return params.map(resolveParam).filter(function (param) { return param != null; });
}
...
var scssContextParser = (function () {
    ...
    if (match) {
      ...
      if (match[1] === '@' && (match[2] === 'function' || match[2] === 'mixin')) {
        ...
        context.params = extractParams(ctxCode);
